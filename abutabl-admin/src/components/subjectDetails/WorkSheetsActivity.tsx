...
const WorkSheetsActivity = () => {
  ...
  const columns = [
    {
      name: t("fileName"),
      selector: (row: any) => (lang === "en" ? row.title_en : row.title_ar),
      width: "20%",
    },
    {
      name: t("scoretopass"),
    ...
    if (lessonId) {
      dispatch(
        ...
        .then((result: any) => {
          result?.workSheets &&
            setRows(
              result?.workSheets?.map((item: any) => {
                return {
                  ...item,
                  status: (
                    <SwitchBox
  ...
  useEffect(() => {
    (async () => {
      await dispatch(getActivities({ type: "worksheets" }));
    })();
  }, []);
  ...
  return (
    <>
              ...
              <>
                ...
                {lessonId && (
                  <Box>
                      ...
                      >
                        {addworksheets && (
                          <Button
                            onClick={() =>
                              navigate(
                                `/subjects/add-worksheet/${param.id}${lessonId ? `?activity_lesson_id=${lessonId}` : ""}`
                              )
                            }
                            ...
                            <div
                              ...
                              onClick={() => {
                                if (viewworksheets) {
                                  ...
                                  // Increased delay to 300ms to ensure file chooser dialog is fully closed
                                  setTimeout(() => {
                                    const newWindow = window.open(`SheetView/${item.id}`, "_blank");
                                    // Fallback if popup is blocked - try again after a longer delay
                                    if (!newWindow) {
                                      setTimeout(() => {
                    ...
                    {rows?.workSheets?.length >= 10 && (
                      <Pagination
                        from={rows?.workSheets?.from}
                        to={rows?.workSheets?.to}
                        lastPage={rows?.workSheets?.last_page}
                        currentPage={rows?.workSheets?.current_page}
                        total={rows?.workSheets?.total}
                        perPage={rows?.workSheets?.per_page}
      ...
      <DeleteConfirmationModal
        ...
        onConfirm={async () => {
          if (contentToDelete) {
            await dispatch(deleteContent(contentToDelete?.id))
              .unwrap()
              .then(() => {
        ...
        onConfirm={async () => {
          if (activityToDelete) {
            await dispatch(deleteActivity(activityToDelete?.id))
              ...
              .then((result: any) => {
                setLessonData({});
                dispatch(getActivities({ type: "worksheets" }));
              })
              .catch((error: any) => {
                console.log(error);
              });
      ...
      />
      <DeleteConfirmationModal
        open={open.deleteSheetModal}
        onClose={() => {
          handleClose("deleteSheetModal");
...
export default WorkSheetsActivity;
