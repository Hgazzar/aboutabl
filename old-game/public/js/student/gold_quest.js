...
answerImages.forEach((img) => {
    img.addEventListener("click", () => {
        ...
        $('#win_gift').empty()
        // console.log(randomImage)   //  ناقض حفظ الجايزه
        let student_coins = document.getElementById('student_coins')
        let coins = 0
        if (randomImage.includes('gold1')) {
            coins = 5
            student_coins.innerText = parseInt(student_coins.innerText) + 5
            $('#win_gift').append(`
                <div class="question" style="height: auto;display: grid;padding-bottom: 10px;padding-top: 10px;max-width: 100% !important;align-items: center;margin-right: 40px;margin-left: 40px;">
                    <h1 style="text-align: center;color: greenyellow;">You win: <span style="text-transform: capitalize;">5 Coins</span></h1>
                </div>
            `)
        } else if (randomImage.includes('gold2')) {
            coins = 10
            student_coins.innerText = parseInt(student_coins.innerText) + 10
            $('#win_gift').append(`
                <div class="question" style="height: auto;display: grid;padding-bottom: 10px;padding-top: 10px;max-width: 100% !important;align-items: center;margin-right: 40px;margin-left: 40px;">
                    <h1 style="text-align: center;color: greenyellow;">You win: <span style="text-transform: capitalize;">10 Coins</span></h1>
                </div>
            `)
        } else if (randomImage.includes('gold3')) {
            coins = 20
            student_coins.innerText = parseInt(student_coins.innerText) + 20
            $('#win_gift').append(`
                <div class="question" style="height: auto;display: grid;padding-bottom: 10px;padding-top: 10px;max-width: 100% !important;align-items: center;margin-right: 40px;margin-left: 40px;">
                    <h1 style="text-align: center;color: greenyellow;">You win: <span style="text-transform: capitalize;">20 Coins</span></h1>
                </div>
            `)
        }
        img.src = randomImage;
...
function answer_gold_quest_question(answer, index) {
            ...
            setTimeout(function () {
                if (parseInt(question_num.innerText) !== questions.length) {
                    ...
                    <div class="answers">
                        ...
                        <div class="answerCard" id="answerCard1" onclick="answer_gold_quest_question('${questions[parseInt(question_num.innerText)].answer1}', 1)">${question1}</div>
                        <div class="answerCard" id="answerCard2" onclick="answer_gold_quest_question('${questions[parseInt(question_num.innerText)].answer2}', 2)">${question2}</div>
                        <div class="answerCard" id="answerCard3" onclick="answer_gold_quest_question('${questions[parseInt(question_num.innerText)].answer3}', 3)">${question3}</div>
                        <div class="answerCard" id="answerCard4" onclick="answer_gold_quest_question('${questions[parseInt(question_num.innerText)].answer4}', 4)">${question4}</div>
                    </div>
