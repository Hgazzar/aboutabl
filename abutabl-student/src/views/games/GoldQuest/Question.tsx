...
export const goldQuestionIdxKey = (id: string) => `interactive_game_q_idx_gq_${id}`;

export default function GoldQuestQuestion() {
	...
	const afterRound = ({ correct }: { correct: boolean }) => {
		if (!gameId) return;
		if (correct) {
			navigate(`/games/${gameId}/gold-quest/gifts`);
		}
	};
