...
/**
 * F-045B — Result shows Runtime submit percentage (server-graded), not client score.
 */
const Result = () => {
	...
	const location = useLocation() as {
		state?: { runtimeResult?: { percentage?: number; score?: number; max_score?: number; pending_manual?: boolean } };
	};

	const runtime = location.state?.runtimeResult || detailsQuiz?.result;
	const displayScore =
		runtime?.percentage != null
			? Math.round(Number(runtime.percentage))
			: Number(score || 0);
	const maxScore =
		runtime?.max_score != null ? Number(runtime.max_score) : null;
	const pendingManual = Boolean(runtime?.pending_manual);
				...
				<div className="w-full h-[85vh] md:p-0 flex justify-center relative">
					...
					<Box className="flex flex-col justify-center text-center">
						<div className="border border-Platinum bg-white w-full  md:p-10 flex flex-col justify-start items-center gap-6  rounded-[25px] shadow-custom-sm mx-2 ">
							...
							<Text className="font-bold  text-2xl text-LightSeaGreen flex">
								{' '}
								<img src={Star} className="pr-2" alt="" />
								{displayScore}
								{pendingManual ? '' : '%'}
							</Text>
